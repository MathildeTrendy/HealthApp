...
router.get('/patients', requireRole('psychiatrist'), csrfProtection, async (req, res) => {
    const { rows } = await pool.query(
        'SELECT id, first_name, last_name FROM users WHERE psychiatrist_id = $1',
        [req.user.id]
    );
    res.render('psychiatrist_patients', {
        patients: rows,
        user: req.user,
...
router.get('/notes/:patientId', requireRole('psychiatrist'), csrfProtection, async (req, res) => {
    ...
    const check = await pool.query(
        'SELECT id, first_name, last_name FROM users WHERE id = $1 AND psychiatrist_id = $2',
        [patientId, req.user.id]
    );
    if (check.rows.length === 0) return res.status(403).send("Ingen adgang til denne patient");

    const notes = await pool.query(
        'SELECT content, created_at FROM notes WHERE userid = $1 ORDER BY created_at DESC',
        [patientId]
    );

    res.render('psychiatrist_notes', {
        notes: notes.rows,
        patient: check.rows[0],
        user: req.user,
        csrfToken: req.csrfToken()
    });
});
